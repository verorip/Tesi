...
Inoltre per la gestione distribuita di progetti softwave e di versionamento del cofice \`e
stata usat la piattaforma GitLab \cite{git-website}.
...
(come ad esempio l'"if" e l'"else"), quindi tutti i controllli di questo tipo sono data-driven.\\
...
(cioè di quelle che riconsegnano il controllo al chiamante prima della loro terminazione).
...
In ECMAJavascript 6 (ES6) \`e stato introdotto il concetto di Promises, ovvero al posto di passare una funzione Callback, viene una Promise(Promessa),
...
Come molti altri linguaggi supporta pacchetti anche sviluppati da terzi, salvati in una repository pubblica o da
piattaforme esterne.
