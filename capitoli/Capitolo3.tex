...
all'avvio \ref{cap:MMavvio}, la strtuttura del file di configurazione \ref{cap:MMconf}, come viene implementata un'applicazione \ref{cap:app}
...
\item Le API per i moduli, i metodi e funzionalit\`a esposti dla MM per poter itegrare i moduli creati
...
\section{Avvio ed Escuzione}\label{cap:MMavvio}
...
La pagina costruita e l'output dei moduli sono un insieme di Document Object Model (DOM), ovvero oggetti HTML che compongo una pagina web, che vengono mostrati
...
I valori di queste possono essere sovrascritti modificando il campo \textit{config} del relativo modulo nel file di configurazione del MagicMirror.
...
il messaggio, il secondo parametro \`e opzionale ed pu\`o essere usato per inserire il corpo del messaggio.
